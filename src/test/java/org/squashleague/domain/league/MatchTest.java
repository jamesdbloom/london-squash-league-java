...
public class MatchTest {
    ...
    public void shouldCalculatePoints() {
        ...
        assertEquals(new Double(1.17), matchOne.getPlayerOnePoints()); // (3 + .5) / 3.0  = 1.1666666666666667
        assertEquals(new Double(0.4), matchOne.getPlayerTwoPoints()); // (1 + .2) / 3.0  = 0.39999999999999997

        Match matchTwo = new Match().withDivision(new Division().withName(1)).withScore("100-200");
        assertEquals(new Double(11.0), matchTwo.getPlayerOnePoints()); // (1 + 10) / 1.0 = 11.0
        assertEquals(new Double(23.0), matchTwo.getPlayerTwoPoints()); // (3 + 20) / 1.0 = 23.0

        Match matchThree = new Match().withDivision(new Division().withName(5)).withScore("1-0");
        assertEquals(new Double(0.62), matchThree.getPlayerOnePoints()); // (3 + 0.1) / 5.0 = 0.62
        assertEquals(new Double(0.2), matchThree.getPlayerTwoPoints()); // (1 + 0.0) / 5.0 = 0.2

        Match matchFour = new Match().withDivision(new Division().withName(5)).withScore("1-1");
        assertEquals(new Double(0.42), matchFour.getPlayerOnePoints()); // (2 + 0.1) / 5.0 = 0.42000000000000004
        assertEquals(new Double(0.42), matchFour.getPlayerTwoPoints()); // (2 + 0.1) / 5.0 = 0.42000000000000004
    }
}
