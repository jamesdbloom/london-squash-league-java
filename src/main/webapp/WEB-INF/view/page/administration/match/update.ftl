            ...
            <#if (players?size > 0)>
                <select id="playerOne" name="playerOne" required="required" title="${environment.getProperty("validation.match.playerOne")}">
                    ...
                    <#list players as player>
                        <option value="${player.id}" <#if (match.playerOne?? && match.playerOne.id == player.id)>selected="selected"</#if>>${player.league.name} <#if player.currentDivision??>&ndash; ${player.currentDivision.name}</#if>&ndash; ${player.user.name}</option>
                    </#list>
        ...
        <p class="select">
            ...
            <#if (players?size > 0)>
                <select id="playerTwo" name="playerTwo" required="required" title="${environment.getProperty("validation.match.playerTwo")}">
                    <option value="">${environment.getProperty("message.general.please_select")}</option>
                    <#list players as player>
                        <option value="${player.id}" <#if (match.playerTwo?? && match.playerTwo.id == player.id)>selected="selected"</#if>>${player.league.name} <#if player.currentDivision??>&ndash; ${player.currentDivision.name}</#if>&ndash; ${player.user.name}</option>
                    </#list>
                </select> <span class="error_message"></span>
            </#if>
        </p>
